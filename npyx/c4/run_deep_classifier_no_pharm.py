...
def cross_validate(
    ...
    n_splits = len(dataset) if loo else 10
...
def main(
    ...
    datasets_abs = [os.path.join(args.data_folder, "merged_no_pharm_labels_2024_10_31.h5")]
    ...
    _, dataset_class = extract_and_check(
        *datasets_abs,
        ...
        _label="no_pharm_label",  #! Larger subset than ground_truth
    )

    if args.check_stability:
        # Modifies the quality checks mask in place to also filter by stability
        dataset_class.filter_by_stability(std_threshold=3)
    ...
    if args.use_grcs:
        if args.mli_clustering:
            from npyx.datasets import CORRESPONDENCE_MLI_CLUSTER as CORRESPONDENCE
            from npyx.datasets import LABELLING_MLI_CLUSTER as LABELLING
        else:
            from npyx.datasets import CORRESPONDENCE, LABELLING
        granule_mask = np.zeros(len(checked_dataset.targets), dtype=bool)
    else:
    ...
    if args.use_grcs:
        features_suffix += "_with_grcs"
    if args.check_stability:
        features_suffix += "_stability_filtered"
    ...
    model_name = f"no_pharm_deep_semisup{suffix}{augmented}"
    ...
    # Save cell counts pre and post checks
    cell_counts = pd.Series(targets).replace(to_replace=CORRESPONDENCE).value_counts()
    cell_counts.to_csv(os.path.join(save_folder, "cell_counts.csv"), header=False)
    cell_counts_pre_checks = pd.Series(dataset_class.targets).replace(to_replace=CORRESPONDENCE).value_counts()
    cell_counts_pre_checks.to_csv(os.path.join(save_folder, "cell_counts_pre_checks.csv"), header=False)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--use_grcs",
        action="store_true",
        help="Whether to include granule cells during training.",
    )
    parser.set_defaults(use_grcs=False)

    parser.add_argument(
        "--check-stability",
        action="store_true",
        help="Whether to filter out neurons based on their stability over the recording period. Used in review, "
        "as an additional quality filter for neurons whose label was determined without pharmacology.",
    )
    parser.set_defaults(check_stability=False)
